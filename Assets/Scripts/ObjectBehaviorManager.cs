...
public class ObjectBehaviorManager : MonoBehaviour
{
    ...
    {
        objectToBehaveOn = objectToBeahveOn.gameObject;
        ...
        if (categories.drive)
        {
            ...
            if (objectToBeahveOn.className != "car" && objectToBeahveOn.className != "van")
            {
        ...
        if(categories.afraid)
        {
            StartCoroutine(afraid(objectToBeahveOn.gameObject));
        }
    ...
    IEnumerator moveCar(GameObject objectToBehaveOn)
    {
        ...
        if(objectToBehaveOn.transform.localScale.x<0)
        nameOfChildObject = "realRight";
        else
            nameOfChildObject = "realLeft"; 
        childLocation = "/" + nameOfParentObject + "/" + nameOfChildObject;
        GameObject rightCorner = GameObject.Find(childLocation);
        ...
        while (indexOfLine < LineManager.instance.line.positionCount - 2)
        {
            ...
            SpeakManager.instance.Speak("♪♪", false, 0);
            SpeakManager.instance.Speak("♪♪♪♪", false, 0);
            smoke.GetComponent<ParticleFollow>().followObject(new Vector3(rightCorner.transform.position.x+0.5f,bottom.transform.position.y+0.5f,1));
        ...
        yield return new WaitForSeconds(2f);
        SpeakManager.instance.stop = false;
    ...
    IEnumerator afraid(GameObject objectToBehaveOn)
    {
    ...
    int nbrOfRocksToThrow = 2, rockCounter = 0;
    public void throwRock()
    {
        float speed = 10;
        Transform handPos = PlayerController.instance.hand;
        GameObject rockClone = Instantiate(rock);
        rockClone.transform.position = handPos.position;
        rockClone.GetComponent<Rigidbody>().AddForce(new Vector3(-100*speed, 60));
        rockClone.GetComponent<Rigidbody>().useGravity = true;
        Destroy(rockClone, 5f);
        rockCounter++;
        if(rockCounter>=nbrOfRocksToThrow)
        {
            objectToBehaveOn.GetComponent<Rigidbody>().velocity = new Vector3(-100, 0, 0);
            Destroy(objectToBehaveOn,1) ;
            PlayerController.instance.changeLinePosition(false);
        }
